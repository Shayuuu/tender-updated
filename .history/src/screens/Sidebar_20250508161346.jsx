...
const Sidebar = ({ currentRoute }) => {
  ...
  // Determine if TaskList subsections should be visible
  const taskListSubsectionRoutes = [
    { path: '/tasklist', exact: true },
    { path: '/tasks', exact: true }
  ];

  const isTaskListSubsectionOpen = taskListSubsectionRoutes.some(route =>
    typeof route === 'string'
      ? location.pathname === route || currentRoute === route
      : matchPath({ path: route.path, exact: route.exact }, location.pathname) ||
        (currentRoute && matchPath({ path: route.path, exact: route.exact }, currentRoute))
  );
  console.log('Sidebar: isTaskListSubsectionOpen:', isTaskListSubsectionOpen);
    ...
    <div className="sidebar-container">
      ...
      >
        ...
        <nav className="sidebar-nav">
          ...
          <div className="sidebar-tasklist-section">
            <Link
              to="/tasklist"
            ...
            {(isSidebarPinned || isSidebarExpanded) && isTaskListSubsectionOpen && (
              <div className="sidebar-tasklist-subsection">
                <Link
                  to="/tasks"
                  className={`sidebar-sub-link sidebar-sub-link-task-summary ${matchPath({ path: '/tasks', exact: true }, location.pathname) ? 'active' : ''}`}
                >
                  {(isSidebarPinned || isSidebarExpanded) && <span>Task Summary</span>}
                </Link>
              </div>
            )}
          </div>
