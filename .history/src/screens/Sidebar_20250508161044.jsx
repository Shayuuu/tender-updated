...
const Sidebar = ({ currentRoute }) => {
  ...
  // Determine if Task Summary subsections should be visible
  const taskSummarySubsectionRoutes = [
    { path: '/tasks', exact: true },
    { path: '/tasks/employee', exact: true },
    { path: '/tasks/project', exact: true },
    { path: '/tasks/summary', exact: true }
  ];

  const isTaskSummarySubsectionOpen = taskSummarySubsectionRoutes.some(route =>
    typeof route === 'string'
      ? location.pathname === route || currentRoute === route
      : matchPath({ path: route.path, exact: route.exact }, location.pathname) ||
        (currentRoute && matchPath({ path: route.path, exact: route.exact }, currentRoute))
  );
  console.log('Sidebar: isTaskSummarySubsectionOpen:', isTaskSummarySubsectionOpen);
      ...
      <div className={`hamburger-menu ${isMobileMenuOpen ? 'open' : ''}`}>
        ...
        {isMobileMenuOpen && (
          <nav ref={dropdownRef} className="mobile-nav-dropdown">
            ...
            <Link
              to="/tasklist"
              className={`mobile-nav-link ${location.pathname === '/tasklist' ? 'active' : ''}`}
              onClick={toggleMobileMenu}
            >
      ...
      >
        ...
        <nav className="sidebar-nav">
          ...
          <div className="sidebar-task-summary-section">
            ...
            {(isSidebarPinned || isSidebarExpanded) && isTaskSummarySubsectionOpen && (
              <div className="sidebar-task-summary-subsection">
                <Link
                  to="/tasks/employee"
                  className={`sidebar-sub-link sidebar-sub-link-employee ${matchPath({ path: '/tasks/employee', exact: true }, location.pathname) || location.pathname === '/tasks' ? 'active' : ''}`}
                >
                  {(isSidebarPinned || isSidebarExpanded) && <span>Employee</span>}
                </Link>
                <Link
                  to="/tasks/project"
                  className={`sidebar-sub-link sidebar-sub-link-project ${matchPath({ path: '/tasks/project', exact: true }, location.pathname) ? 'active' : ''}`}
                >
                  {(isSidebarPinned || isSidebarExpanded) && <span>Project</span>}
                </Link>
                <Link
                  to="/tasks/summary"
                  className={`sidebar-sub-link sidebar-sub-link-summary ${matchPath({ path: '/tasks/task-summary', exact: true }, location.pathname) ? 'active' : ''}`}
                >
                  {(isSidebarPinned || isSidebarExpanded) && <span>Summary</span>}
                </Link>
              </div>
            )}
          </div>
