...
const Expenses = () => {
  ...
  useEffect(() => {
    ...
    if (data) {
      setExpensesData(Array.isArray(data.expenses) ? data.expenses : []);
      setRaBills(data.account && Array.isArray(data.account.bills) ? data.account.bills.map(bill => bill.raNumber) : []);
    }
  }, [id]);
