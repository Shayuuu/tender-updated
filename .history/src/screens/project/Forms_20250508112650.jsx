...
const Forms = () => {
  ...
  useEffect(() => {
    ...
    let data = localProject ? localProject.forms : project?.forms;
    ...
    if (!data) {
      ...
      // If loading existing data, ensure attachments don't have blob URLs
      const loadedData = {
        ...data,
      ...
      setIsNewForm(false);
      const fields = Object.keys(loadedData).filter((key) => key !== 'id');
  ...
  const handleSave = () => {
    // Prepare formData for saving by removing blob URLs from attachments
    const saveData = {
      ...formData,
    ...
    if (projectIndex !== -1) {
      ...
      localData[projectIndex] = newProject;
    } else {
      // Add new project
