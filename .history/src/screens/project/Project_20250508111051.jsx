...
const Project = () => {
  ...
  const handleNewProject = () => {
    ...
    // Navigate to the Forms page with the new ID
  ...
  // Merge projectData and localData, prioritizing localData for overlapping IDs
  const mergedData = [];
  const localDataIds = new Set(localData.map(item => item.id));

  // Add items from projectData if they don't exist in localData
  ...
  // Add all items from localData (they take precedence)
  mergedData.push(...localData);

  // Normalize the merged data
  const combinedData = mergedData.map(normalizeProject);

  // Filter data based on search term, accounting for normalized structure
