...
import { faHome, faUserTie, faFolderOpen, faTasks, faSignOutAlt, faBars, faWindowRestore, faChevronDown, faChevronUp, faFileAlt, faCalendarAlt, faUsers, faChartLine } from '@fortawesome/free-solid-svg-icons'
import './Sidebar.css'
...
const Sidebar = ({ currentRoute }) => {
  ...
  const bidderSubsectionRoutes = ['/form-details', '/extension-dates', '/bidder-details', '/milestone-tracker']
  const isBidderSubsectionOpen = bidderSubsectionRoutes.includes(location.pathname) || bidderSubsectionRoutes.includes(currentRoute)
          ...
          <div className="sidebar-bidder-section">
            ...
            >
              ...
              {(isSidebarPinned || isSidebarExpanded) && (
                <FontAwesomeIcon
                  icon={isBidderSubsectionOpen ? faChevronUp : faChevronDown}
                  className="sidebar-chevron-icon"
                />
              )}
            </Link>
            {isBidderSubsectionOpen && (
              <div className="sidebar-bidder-subsection">
                <Link
                  to="/form-details"
                  className={`sidebar-sub-link sidebar-sub-link-form-details ${location.pathname === '/form-details' ? 'active' : ''}`}
                >
                  <FontAwesomeIcon icon={faFileAlt} className="sidebar-sub-icon" />
                  {(isSidebarPinned || isSidebarExpanded) && <span>Form<br />Details</span>}
                </Link>
                <Link
                  to="/extension-dates"
                  className={`sidebar-sub-link sidebar-sub-link-extension-dates ${location.pathname === '/extension-dates' ? 'active' : ''}`}
                >
                  <FontAwesomeIcon icon={faCalendarAlt} className="sidebar-sub-icon" />
                  {(isSidebarPinned || isSidebarExpanded) && <span>Extension<br />Dates</span>}
                </Link>
                <Link
                  to="/bidder-details"
                  className={`sidebar-sub-link sidebar-sub-link-bidder-details ${location.pathname === '/bidder-details' ? 'active' : ''}`}
                >
                  <FontAwesomeIcon icon={faUsers} className="sidebar-sub-icon" />
                  {(isSidebarPinned || isSidebarExpanded) && <span>Bidder<br />Details</span>}
                </Link>
                <Link
                  to="/milestone-tracker"
                  className={`sidebar-sub-link sidebar-sub-link-milestone-tracker ${location.pathname === '/milestone-tracker' ? 'active' : ''}`}
                >
                  <FontAwesomeIcon icon={faChartLine} className="sidebar-sub-icon" />
                  {(isSidebarPinned || isSidebarExpanded) && <span>Milestone<br />Tracker</span>}
                </Link>
              </div>
            )}
          </div>
