...
const Sidebar = ({ currentRoute }) => {
  ...
  const sectionIdMatch = location.pathname.match(/\/(bidders|extension-dates|bidder-details|milestone-tracker|projects|project-completion-dates|project-running-account|project-expenses|project-variations|project-notes)\/(\d+)/)
  ...
  const projectSubsectionRoutes = [
    { path: '/projects/:id', exact: true },
    { path: '/project-completion-dates/:id', exact: true },
    { path: '/project-running-account/:id', exact: true },
    { path: '/project-expenses/:id', exact: true },
    { path: '/project-variations/:id', exact: true },
    { path: '/project-notes/:id', exact: true }
  ]
          ...
          <div className="sidebar-project-section">
            ...
            {(isSidebarPinned || isSidebarExpanded) && isProjectSubsectionOpen && (
              <div className="sidebar-project-subsection">
                {/* Forms Subsection */}
                <Link
                  to={location.pathname.match(/^\/projects\/[^/]+$/) ? location.pathname : `/projects/${sectionId}`}
                  className={`sidebar-sub-link sidebar-sub-link-forms ${matchPath({ path: '/projects/:id', exact: true }, location.pathname) ? 'active' : ''}`}
                >
                  {(isSidebarPinned || isSidebarExpanded) && <span>Forms</span>}
                </Link>
                {/* Completion Dates Subsection */}
                <Link
                  to={location.pathname.match(/^\/project-completion-dates\/[^/]+$/) ? location.pathname : `/project-completion-dates/${sectionId}`}
                  className={`sidebar-sub-link sidebar-sub-link-completion-dates ${matchPath({ path: '/project-completion-dates/:id', exact: true }, location.pathname) ? 'active' : ''}`}
                >
                  {(isSidebarPinned || isSidebarExpanded) && <span>Completion Dates</span>}
                </Link>
                {/* Running Account Subsection */}
                <Link
                  to={location.pathname.match(/^\/project-running-account\/[^/]+$/) ? location.pathname : `/project-running-account/${sectionId}`}
                  className={`sidebar-sub-link sidebar-sub-link-running-account ${matchPath({ path: '/project-running-account/:id', exact: true }, location.pathname) ? 'active' : ''}`}
                >
                  {(isSidebarPinned || isSidebarExpanded) && <span>Running Account</span>}
                </Link>
                {/* Expenses Subsection */}
                <Link
                  to={location.pathname.match(/^\/project-expenses\/[^/]+$/) ? location.pathname : `/project-expenses/${sectionId}`}
                  className={`sidebar-sub-link sidebar-sub-link-expenses ${matchPath({ path: '/project-expenses/:id', exact: true }, location.pathname) ? 'active' : ''}`}
                >
                  {(isSidebarPinned || isSidebarExpanded) && <span>Expenses</span>}
                </Link>
                {/* Variations Subsection */}
                <Link
                  to={location.pathname.match(/^\/project-variations\/[^/]+$/) ? location.pathname : `/project-variations/${sectionId}`}
                  className={`sidebar-sub-link sidebar-sub-link-variations ${matchPath({ path: '/project-variations/:id', exact: true }, location.pathname) ? 'active' : ''}`}
                >
                  {(isSidebarPinned || isSidebarExpanded) && <span>Variations</span>}
                </Link>
                {/* Notes Subsection */}
                <Link
                  to={location.pathname.match(/^\/project-notes\/[^/]+$/) ? location.pathname : `/project-notes/${sectionId}`}
                  className={`sidebar-sub-link sidebar-sub-link-notes ${matchPath({ path: '/project-notes/:id', exact: true }, location.pathname) ? 'active' : ''}`}
                >
