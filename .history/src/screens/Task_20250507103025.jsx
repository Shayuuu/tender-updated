...
const Task = () => {
  ...
  const handleDeleteForm = (id) => {
    ...
    const updatedExtensions = storageData.extensions.filter(entry => entry.id !== id)
    ...
    localStorage.setItem('customExtensionData', JSON.stringify(updatedExtensions))
    ...
    if (dateFields.includes(field) && value) {
      ...
      return value.length > 0 ? (
        <ul>
          {value.map((ext, index) => (
            <li key={index}>
              Extension {ext.extensionNo}: {formatField('date', ext.date)}
            </li>
          ))}
        </ul>
      ) : '-'
  ...
  const getFormData = (id) => {
    ...
    const extensionEntry = storageData.extensions.find(entry => entry.id === id) || { extensions: [] }
