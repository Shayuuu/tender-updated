...
const Task = () => {
  ...
  const getFormData = (id) => {
    ...
    // Categorize fields from bidderEntry
    Object.entries(bidderEntry).forEach(([key, value]) => {
      if (key === 'id' || key === 'extensionDates') return; // Exclude id and extensionDates
    ...
    // Categorize fields from projectEntry
    const formsFields = projectEntry.forms || {};
    Object.entries(projectEntry).forEach(([key, value]) => {
      if (key === 'id' || key === 'forms') return; // Exclude id and forms (handled separately)
                  ...
                  <div className="task-card">
                    ...
                    {expandedFormId === formId && (
                      <div className="task-card-details">
                        <h3>Form Details</h3>
                        {getFormData(formId).map((group, index) => (
                          group.hasData && (
