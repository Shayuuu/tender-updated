...
const BidderDetails = () => {
  ...
  useEffect(() => {
    ...
    if (data) {
      ...
      // Initialize empty for new forms
      setTechnicalOpeningDate('')
  ...
  const handleDeleteBidder = (index) => {
    const updatedEditBidders = editBidders.filter((_, i) => i !== index)
    setEditBidders(updatedEditBidders)
  }
  ...
  const handleSaveAdd = () => {
    if (newBidder.bidderName && newBidder.financialBidAmount) {
      ...
      const updatedBidders = [...bidders, newEntry]
      ...
      if (bidderIndex !== -1) {
        ...
        localData[bidderIndex].bidders = updatedBidders
      } else {
        // Create new entry if it doesn't exist
        const newBidderEntry = { id: parseInt(id), bidders: updatedBidders, technicalOpeningDate: '', financialOpeningDate: '' }
        localData.push(newBidderEntry)
      }
  ...
  const handleSaveEdit = () => {
    // Only require dates to save dates, don't require bidders
    if (editDates.technical && editDates.financial) {
      setTechnicalOpeningDate(editDates.technical)
      setFinancialOpeningDate(editDates.financial)

      // Update or create entry in localStorage for dates
      const localData = JSON.parse(localStorage.getItem('customBidderData')) || []
      const bidderIndex = localData.findIndex((b) => b.id === parseInt(id))
      if (bidderIndex !== -1) {
        localData[bidderIndex].technicalOpeningDate = editDates.technical
        localData[bidderIndex].financialOpeningDate = editDates.financial
      } else {
        const newBidderEntry = {
          id: parseInt(id),
          technicalOpeningDate: editDates.technical,
          financialOpeningDate: editDates.financial,
          bidders: []
        }
        localData.push(newBidderEntry)
      }
    ...
    // Save bidders if there are any changes
    if (editBidders.length >= 0) {
      // Reassign bidderNo to maintain sequential order after deletion
      const updatedBidders = editBidders.map((bidder, index) => ({
        ...bidder,
        bidderNo: index + 1
      }))
      setBidders(updatedBidders)

      // Update or create entry in localStorage for bidders
      const localData = JSON.parse(localStorage.getItem('customBidderData')) || []
      const bidderIndex = localData.findIndex((b) => b.id === parseInt(id))
      if (bidderIndex !== -1) {
        localData[bidderIndex].bidders = updatedBidders
        ...
        <div className="bidder-details-content">
          ...
          <div className="bidder-details-list-container">
            <div className="bidder-details-list-header">
              ...
              {isEditing && <div className="bidder-details-list-column">Actions</div>}
            </div>
            <div className="bidder-details-list">
              {bidders.length === 0 && !isAdding && (
                <div className="bidder-details-empty">
              ...
              {rankedBidders.map((bidder, index) => (
                <div key={index} className="bidder-details-card">
                  <div className="bidder-details-list-column">
                    ...
                    {isEditing ? editBidders[index]?.bidderNo : bidder.bidderNo}
                  </div>
                  <div className="bidder-details-list-column">
                  ...
                  {isEditing && (
                    <div className="bidder-details-list-column">
                      <span className="bidder-details-label-mobile">Actions:</span>
                      <button
                        className="bidder-details-btn bidder-details-delete-btn"
                        onClick={() => handleDeleteBidder(index)}
                      >
                        Delete
                      </button>
                    </div>
